...
namespace delaunator {
    ...
    Delaunator::Delaunator(std::vector<double> const& in_coords)
        : coords(in_coords), triangles(), halfedges(), hull_prev(), hull_next(), hull_tri(), hull_start(), m_hash(), m_center_x(), m_center_y(), m_hash_size(), m_edge_stack() {
        ...
        for (std::size_t k = 0; k < n; k++) {
            ...
            if (e == start) {
                while (
                    q = hull_prev[e], orient(x, y, coords[2 * q], coords[2 * q + 1], coords[2 * e], coords[2 * e + 1])) {
    ...
    std::size_t Delaunator::legalize(std::size_t a) {
        ...
        while (true) {
            ...
            * (p1 is inside the circumcircle of [p0, pl, pr]), flip them, * then do the same check/flip recursively for the new pair of triangles
            *
            *           pl                    pl
            *          /||\                  /  \
            *       al/ || \bl            al/    \a
    ...
    inline std::size_t Delaunator::hash_key(const double x, const double y) const {
        ...
        return fast_mod(
            static_cast<std::size_t>(std::llround(std::floor(pseudo_angle(dx, dy) * static_cast<double>(m_hash_size)))), m_hash_size);
    }
    ...
    std::vector<point> getTriangleEdges(std::vector<double> points, Delaunator delaunay) {
        ...
        for (int e = 0; e < delaunay.triangles.size(); e++) {
            if (e > delaunay.halfedges[e]) {
                edges.push_back(point{(int)points[delaunay.triangles[e]], (int)points[delaunay.triangles[nextHalfedge(e)]]});
            }
        }
