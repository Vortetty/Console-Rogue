...
public:
    ...
    ruleAutomata(vec2 gridSize, std::deque<std::deque<bool>> grid);
    ruleAutomata(std::initializer_list<uint8_t> birth, std::initializer_list<uint8_t> survival);
    ruleAutomata(std::initializer_list<uint8_t> birth, std::initializer_list<uint8_t> survival, vec2 gridSize, std::deque<std::deque<bool>> grid);

    void setRules(std::initializer_list<uint8_t> birth, std::initializer_list<uint8_t> survival);
    void setGrid(std::deque<std::deque<bool>> grid);
